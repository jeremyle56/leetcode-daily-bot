...
client.on('ready', (c) => {
    ...
    client.guilds.cache.forEach(async (g) => {
        ...
        if (channel && channel.isTextBased()) {
            ...
            const question = daily.questionId + ". " + daily.title;
            const difficulty = daily.difficulty === "Easy" ? "🟩" : daily.difficulty === "Medium" ? "🟨" : "🟥";

            // Send the problem to the channel
            const message = await channel.send(`${difficulty} **[DAILY]** [${question}](${daily.link})`);
